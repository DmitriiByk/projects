...
export const api = {
  ...
  skillContent: (id) => req("GET", `/api/skills/${encodeURIComponent(id)}/content`),
  saveSkill: (id, content) => req("PUT", `/api/skills/${encodeURIComponent(id)}/content`, { content }),
  assistSkill: (id, content, request) => req("POST", `/api/skills/${encodeURIComponent(id)}/assist`, { content, request }),
  imageStatus: () => req("GET", "/api/image/status"),
  generateImage: (body) => req("POST", "/api/image/generate", body),
  translate: (items) => req("POST", "/api/translate", { items }),
