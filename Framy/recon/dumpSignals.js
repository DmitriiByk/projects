...
// async из-за follow-into-main (L3). Вызывать: await dumpSignals(target).
async function dumpSignals(target) {
  const layers = [], reuse = {}, visited = new Set();
  async function walk(n, d) {
    if (d > 8 || layers.length > 90) return;
    layers.push(layerSig(n, d));
    if (n.type === 'INSTANCE') {
      let mc = null;
      try { mc = n.getMainComponentAsync ? await n.getMainComponentAsync() : n.mainComponent; } catch (e) {}
      if (mc) {
        reuse[mc.name] = (reuse[mc.name] || 0) + 1;
        // L3 follow-into-main: тонкая обёртка-инстанс (≤1 собственный ребёнок) → проваливаемся в мастер,
        // где живёт реальный декор. visited-set от циклов.
        const ownKids = n.children || [];
        if (ownKids.length <= 1 && !visited.has(mc.id)) {
          visited.add(mc.id);
          if (mc.children) for (const c of mc.children) await walk(c, d + 1);
          return; // дети мастера уже покрыты — собственный (пустой) подграф инстанса не нужен
        }
      }
    }
    if (n.children) for (const c of n.children) await walk(c, d + 1);
  }
  await walk(target, 0);
  ...
  // T16: растровая нарезка (baked) — IMAGE scaleMode=CROP на слоях part/L/M/R. Школа Dragonia.
  const sliceNameRe = /part=(left|center|middle|right)|^(l|left|middle|center|r|right)$/i;
  const rasterSlice = all.some(l => sliceNameRe.test((l.name || '').trim()) &&
    Array.isArray(l.fills) && l.fills.some(f => f.type === 'IMAGE' && f.scaleMode === 'CROP'))
    || all.filter(l => Array.isArray(l.fills) && l.fills.some(f => f.type === 'IMAGE' && f.scaleMode === 'CROP')).length >= 3;

  const flags = {
    stretches_horizontally: root.layoutMode === 'HORIZONTAL',
    is_three_slice: sliced,
    is_raster_slice: rasterSlice,        // T16: baked-нарезка (реюз ассета, не перерисовка)
